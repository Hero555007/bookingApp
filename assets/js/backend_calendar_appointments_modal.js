...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        $('#manage-appointment #save-appointment').click(function () {
            ...
            var startDatetime = $dialog.find('#start-datetime').datepicker('getDate').toString('yyyy-MM-dd') + " " + $dialog.find('#startTimeList :selected').val() + ":00";
            console.log("starttime", startDatetime);
            var bufendDate = $dialog.find('#end-datetime').val();
            var endDatetime = bufendDate.substr(0,4) + "-" + bufendDate.substr(5,2) + "-" + bufendDate.substr(8,2) + bufendDate.substr(10,6) + ":00";
            if (Date.parse(endDatetime) == null)
            {
                console.log("startendtime", startDatetime, endDatetime,Date.parse(endDatetime));
                endDatetime = bufendDate.substr(6,4) + "-" + bufendDate.substr(3,2) + "-" + bufendDate.substr(0,2) + bufendDate.substr(10,6) + ":00";
            }
            // console.log("startendtime", startDatetime, endDatetime);
            var appointment = {
        ...
        $('#insert-appointment').click(function () {
            ...
            // $dialog.find('#start-datetime').val(GeneralFunctions.formatDate(start, GlobalVariables.dateFormat, true).substr(0,10));
            // $dialog.find('#end-datetime').val(GeneralFunctions.formatDate(start.addMinutes(serviceDuration),
            //     GlobalVariables.dateFormat, true));

            $dialog.find('#startTimeList').children().remove();

            var appointmentID;
            if ($dialog.find('#appointment-id').val() !== '') {
                // Set the id value, only if we are editing an appointment.
                appointmentID = $dialog.find('#appointment-id').val();
            }
            else{
            ...
            var postData = {
                ...
                service_id: $dialog.find('#select-service').val(),
                provider_id: $dialog.find('#select-provider').val(),
                selected_date: $('#start-datetime').datepicker('getDate').toString('yyyy-MM-dd'),
                service_duration: serviceDuration,
                manage_mode: false,
                appointment_id: appointmentID
            };
            console.log("responseforstartDate",postData);
            $.post(postUrl, postData, function (response) {
                console.log("responseforstartDate", response);
                if (!GeneralFunctions.handleAjaxExceptions(response)) {
                    return;
                }
                var i = 0;     
                ...
                $.each(response, function (index, availableHour) {
                    ...
                    console.log("availablehour", availableHour);
                    if (i == 0) {
                        initialhour = availableHour;
                    }
                    $('#startTimeList').append('<option value="'+availableHour+'">' + availableHour + '</option>');
                    i++;
                });
                initialhour = $('#start-datetime').datepicker('getDate').toString('yyyy-MM-dd') + " " + initialhour;
                console.log("endDate",Date.parse(initialhour));
                if (Date.parse(initialhour) != null)
                {
                    $dialog.find('#end-datetime').val(GeneralFunctions.formatDate(Date.parse(initialhour).addMinutes(serviceDuration),
                    GlobalVariables.dateFormat, true));
                }
    ...
    exports.resetAppointmentDialog = function () {
        ...
        $dialog.find('#start-datetime').datepicker({
            ...
            firstDay: 0,
            onSelect: function(datatext, instance) {
                var start = $('#start-datetime').datepicker('getDate').toString('yyyy-MM-dd');
                ...
                $.post(postUrl, postData, function (response) {
                    ...
                    {
                        $('#startTimeList').append('<option>no available</option>');
                        $dialog.find('#end-datetime').val("");
                    }
                    else
                    {
                        $.each(response, function (index, availableHour) {
        ...
        $dialog.find('#startTimeList').on('change', function(){
            var serviceDuration = 0;
            $.each(GlobalVariables.availableServices, function (index, service) {
                if (service.id == $dialog.find('#select-service').val()) {
                    serviceDuration = service.duration;
            ...
            var start = $('#start-datetime').datepicker('getDate').toString('yyyy-MM-dd');
            var initialhour;
            initialhour = $dialog.find('#startTimeList :selected').val();
            initialhour = start + " " + initialhour;
            $dialog.find('#end-datetime').val(GeneralFunctions.formatDate(Date.parse(initialhour).addMinutes(serviceDuration),
            GlobalVariables.dateFormat, true));
        });

        console.log("starttime1", $dialog.find('#start-datetime').val());
