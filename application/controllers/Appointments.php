...
class Appointments extends CI_Controller {
    ...
    {
        $this->load->view('appointments/forgot_password');
    }
    public function index($appointment_hash = '', $email_hash = '')
    {
        if ( ! is_ea_installed())
        {
        ...
        try
        {
            ...
            if ($appointment_hash !== '' && $appointment_hash !== '1')
            {
        ...
        if ($email_hash != "")
        {
            $email = base64_decode(strtr($email_hash, '-_', '+/'));
            $postdata = base64_decode(strtr($this->uri->segment(5), '-_', '+/'));
            $cookie_name = "user";
            setcookie($cookie_name, $email, time() + (86400 * 30), "/"); // 86400 = 1 day
            $res = $this->appointments_model->getUserdata($email);
            $view['user_data'] = $res;
            $view['post_data'] = $postdata;
        }
        ...
        $settingres = $this->settings_model->get_settings();
        $view['setting_data'] = $settingres;
    ...
    protected function _calculate_available_hours(
        ...
        foreach ($empty_periods as $period)
        {
            ...
            $interval = $availabilities_type === AVAILABILITIES_TYPE_FIXED ? (int)$service_duration : 55;
            ...
            {
                $available_hours[] = $current_hour->format('m-d H:i');
    ...
    protected function _get_multiple_attendants_hours(
        ...
        $interval_value = $service['availabilities_type'] == AVAILABILITIES_TYPE_FIXED ? $service['duration'] : '55';
    ...
    $this->load->model('appointments_model');
    $data = $_POST;
    $result = $this->appointments_model->get_last_record();
    $id = $result['0']['id'];
   	$res = $this->appointments_model->appointments_quarr($id, $data);
 }
